...
test('test', async ({ page }) => {
  ...
  const GewässergüteLink = links.find((link) => link.textContent === 'Gewässergüte');
  await GewässergüteLink.click();

  // Add code to save the screenshot
  await page.screenshot({ path: 'Gewässergüte.png' });

  // Add code to save the test results
  await writeFileSync('results.txt', 'Test results saved to disk');
});